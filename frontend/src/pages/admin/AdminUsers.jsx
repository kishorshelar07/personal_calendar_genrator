...
const EMPTY = { usr_id:'', usr_name:'', usr_pass:'', usr_status:0, usr_type:'User', usr_dob:'', usr_email:'', usr_gender:'', usr_remarks:'' };
...
const Modal = ({ mode, data, onSave, onClose }) => {
  ...
  useEffect(() => {
    if (isEdit && data) setForm({ ...EMPTY, ...data, usr_pass:'', usr_dob: data.usr_dob?.substring(0,10) || '' });
    else setForm(EMPTY);
  }, [mode, data]);
  ...
  const handleSubmit = async e => {
    ...
    try {
      if (!isEdit && !/^\d{10}$/.test(form.usr_id)) throw new Error('Mobile number must be 10 digits.');
      const payload = { ...form };
      if (isEdit) { delete payload.usr_id; if (!payload.usr_pass) delete payload.usr_pass; }
  ...
  const F = ({ label, k, type='text', ...rest }) => (
    <div className="form-group">
      ...
      <input className="form-control-custom" type={type} value={form[k]} onChange={s(k)} {...rest} />
    </div>
  );
      ...
      <div className="modal-box">
        <div className="modal-header">
          <span className="modal-title" style={{ color:'var(--admin-accent)' }}>
            <i className="bx bx-user-plus me-2"></i>{isEdit ? 'Edit User' : 'Add New User'}
          </span>
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body">
            ...
            <div className="form-row">
              <F label="Mobile Number (ID) *" k="usr_id" placeholder="10 digits" maxLength={10} disabled={isEdit} required={!isEdit} />
              <F label="Full Name *" k="usr_name" maxLength={35} required />
            </div>
            <div className="form-row">
              <F label={isEdit ? 'New Password (blank = keep)' : 'Password *'} k="usr_pass" type="password" required={!isEdit} />
              <F label="Date of Birth *" k="usr_dob" type="date" required />
            </div>
            <div className="form-row">
              <F label="Email *" k="usr_email" type="email" maxLength={50} required />
              <div className="form-group">
                <label className="form-label">Gender</label>
                <select className="form-control-custom" value={form.usr_gender} onChange={s('usr_gender')}>
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                <select className="form-control-custom" value={form.usr_status} onChange={s('usr_status')}>
              ...
              <div className="form-group">
                ...
                <select className="form-control-custom" value={form.usr_type} onChange={s('usr_type')}>
            ...
            <div className="form-group">
              ...
              <input className="form-control-custom" value={form.usr_remarks} onChange={s('usr_remarks')} maxLength={160} />
            </div>
          </div>
          <div className="modal-footer">
            <button type="button" onClick={onClose} style={{ padding:'9px 20px', borderRadius:8, border:'1px solid var(--border)', background:'none', cursor:'pointer', fontFamily:'Outfit,sans-serif', fontSize:14 }}>Cancel</button>
...
const AdminUsers = () => {
  ...
  const handleDelete = async usr_id => {
    if (!window.confirm(`Delete user ${usr_id} and all their data?`)) return;
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={u.usr_id}>
                  ...
                  <td><span style={{ fontFamily:'monospace', fontSize:13, color:'var(--text-1)' }}>{u.usr_id}</span></td>
                  ...
                  <td>
                    <label className="toggle-switch" title={u.usr_status===1?'Deactivate':'Activate'}>
                  ...
                  <td>
                    <div style={{ display:'flex', gap:6 }}>
                      <button className="btn-icon edit" onClick={() => { setSelUser(u); setModal('edit'); }} title="Edit">
                        <i className="bi bi-pencil"></i>
                      </button>
                      <button className="btn-icon delete" onClick={() => handleDelete(u.usr_id)} title="Delete">
